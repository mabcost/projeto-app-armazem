...
class activity_listaprodutos : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val produtosSeparados = intent.getStringArrayListExtra("ItensDivididos")

        val quantidadeProd = arrayListOf<String>()

        listaProd = findViewById(R.id.listaDeProdutos)

        listViewDosProd.clear()

        if (produtosSeparados != null) {
            for (i in produtosSeparados) {
                for (x in products) {
                    if (i == x[0]) {
                ...
                if (i.length < 5){
                    quantidadeProd.add(i)
                }
            }
        }

        for (m in quantidadeProd) {
            var quantidade = m.toInt()
            if (quantidade > 1000) {
                Toast.makeText(applicationContext, "Quantidade em estoque insuficiente", Toast.LENGTH_LONG).show()
        ...
        btnIniciar.setOnClickListener{
            val intent = Intent(this, activity_leitura::class.java)
            intent.putExtra("itensDivididos2",ArrayList(produtosSeparados))
            intent.putExtra("itensMemoria",products)
            intent.putExtra("qtdItens",ArrayList(quantidadeProd))
            startActivity(intent)
        }
